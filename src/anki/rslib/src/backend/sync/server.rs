...
impl Backend {
  ...
  pub  fn meta(&self, input: MetaRequest) -> Result<SyncMeta> {
   ...
   pub fn col_into_server(&self) -> Result<LocalServer> {
        self.col
            .lock()
            .unwrap()
            .take()
  ...
  pub  fn start(&self, input: StartRequest) -> Result<Graves> {
  ...
  pub  fn apply_graves(&self, input: ApplyGravesRequest) -> Result<()> {
        self.with_sync_server(|server| {
            let rt = Runtime::new().unwrap();
  ...
  pub  fn apply_changes(&self, input: ApplyChangesRequest) -> Result<UnchunkedChanges> {
        self.with_sync_server(|server| {
            let rt = Runtime::new().unwrap();
 ...
 pub   fn apply_chunk(&self, input: ApplyChunkRequest) -> Result<()> {
        self.with_sync_server(|server| {
            let rt = Runtime::new().unwrap();
  ...
  pub  fn sanity_check(&self, input: SanityCheckRequest) -> Result<SanityCheckResponse> {
        self.with_sync_server(|server| {
            let rt = Runtime::new().unwrap();
  ...
  pub  fn finish(&self) -> Result<TimestampMillis> {
   ...
   pub fn abort(&self) -> Result<()> {
    ...
    /// consumed.
   pub fn upload(&self, input: PathBuf) -> Result<()> {
   ...
   pub fn download(&self) -> Result<Vec<u8>> {
    ...
    pub fn sync_server_method_inner(&self, req: SyncRequest) -> Result<Vec<u8>> {
